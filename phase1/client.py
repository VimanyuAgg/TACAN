...
def run():
  ...
  response1 = stub.SendPacket(
    phase1_pb2.RequestMessage(nodeId="2", destinationId="3", message="Hello Dear Server Please forward my request!"))
...
def sendShiftNodeRequest(node, bestNodeClusterHeadId, clusterHeadIp):
  ...
  logger.info(
    "Node: %s sent sendShiftNodeRequest about C:%s to clusterhead:%s" % (node.id, bestNodeClusterHeadId, clusterHeadIp))
